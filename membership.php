...
if ($displayName === '') {
    $displayName = $currentUser['username'] ?? ($currentUser['email'] ?? 'Member');
}
        ...
        try {
            ...
            if (!response.ok || apiError) {
                ...
                const diagnosticLines = [
                    'Payment failed.',
                    '',
                    'Message: ' + (apiError || 'Unable to create payment intent'),
                    'Reference: ' + (details.reference || 'n/a'),
                    'Timestamp (UTC): ' + (details.timestamp_utc || 'n/a'),
                    'Endpoint: ' + (details.endpoint || 'n/a'),
                    'User ID: ' + (details.user_id || 'n/a'),
                    'Amount (cents): ' + (details.amount || 'n/a'),
                    'Currency: ' + (details.currency || 'n/a'),
                    'Payment Type: ' + (details.payment_type || 'n/a'),
                    'HTTP Status: ' + (details.http_status || response.status),
                    '',
                    'Copy and paste this entire message to support.'
                ];

                const enhancedError = new Error(apiError || 'Unable to create payment intent');
                enhancedError.userMessage = diagnosticLines.join('\n');
                throw enhancedError;
            }
            ...
            alert(error.userMessage || ('Payment failed: ' + error.message));
