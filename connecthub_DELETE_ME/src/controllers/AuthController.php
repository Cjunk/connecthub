    ...
    public function login() {
        ...
        try {
            ...
            if ($email !== '') {
                ...
                $user = method_exists($this->userModel, 'authenticateByUsername')
                    ? $this->userModel->authenticateByUsername($username, $password)
                    : false;
            }
            if ($user) {
                ...
                $_SESSION['user_name']     = $user['name'] ?? ($user['first_name'] ?? $user['email']);
                $_SESSION['user_email']    = $user['email'];
                $_SESSION['user_role']     = $user['role'];
                $_SESSION['last_activity'] = time();

                $welcome = $user['first_name'] ?? explode(' ', (string)($_SESSION['user_name']))[0];
                setFlashMessage('success', 'Welcome back, ' . $welcome . '!');
