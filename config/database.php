...
class Database {
    ...
    private function __construct() {
        try {
            ...
            $driverHint = strtolower(trim((string)(
                (defined('DB_DRIVER') ? DB_DRIVER : null)
                ?? $_ENV['DB_DRIVER']
                ?? getenv('DB_DRIVER')
                ?? ''
            )));
            ...
            /*
             * Driver selection:
             * - DB_DRIVER=pgsql forces PostgreSQL
             * - DB_DRIVER=mysql forces MySQL
             * - local/dev + port 5432 defaults to PostgreSQL
             * - production defaults to MySQL unless explicitly overridden
             */
            if (in_array($driverHint, ['pgsql', 'postgres', 'postgresql'], true)) {
                $this->driver = 'pgsql';
            } elseif (in_array($driverHint, ['mysql', 'mariadb'], true)) {
                $this->driver = 'mysql';
            } elseif (!$isProduction && $port === 5432) {
                $this->driver = 'pgsql';
            } else {
                $this->driver = 'mysql';
            }
