...
// Some production schemas may not include optional category tables.
// Fall back to an empty group filter list instead of hard failing the page.
try {
  $allGroups = $group->getAll(['status' => 'active']);
} catch (Exception $e) {
  error_log('Events page group filter load failed: ' . $e->getMessage());
  $allGroups = [];
}
